...
beer_awards %>% 
  ...
  unnest(cols = c(x, y)) %>%
  ggplot() +
  geom_rect(aes(xmin = -18, ymin = pos*4 - 1.4,
  ...
  geom_text(aes(-14.5, pos*4, label = state), family = "Futura Md BT", fontface = "bold", color = "#FFFFFF", hjust = 0.5, size = 8.5, check_overlap = TRUE) +
  geom_text(aes(n/25, pos*2, label = n), family = "Futura Bk BT", color = "#FFFFFF", size = 4, check_overlap = TRUE) +
  scale_x_continuous(breaks = seq(0, 80, 20), labels = seq(0, 1000, 250)) +
  labs(title = "AWARD WINNING BEERS",
       subtitle = "GABF Medals by State (1987-2020)",
       caption = "• Produced by @CSHoggard | #TidyTuesday Week 43 | Data: Great American Beer Festival • ") +
  theme_minimal() +
  theme(plot.title = element_text(colour = "#005F85", family = "Futura Md BT", face = "bold", size = 42, hjust = 0.5, margin = margin(10,0,20,0)),
        plot.subtitle = element_text(colour = "#6F94AA", family = "Futura Md BT", face = "bold", size = 24, hjust = 0.5, margin = margin(0,0,20,0)),
        plot.caption = element_text(colour = "#626262", family = "Bellota Text", face = "bold", size = 10, hjust = 0.5, margin = margin(30,0,0,0)),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        axis.text.x = element_text(family = "Futura Md BT"),
        axis.text.y = element_blank(),
        panel.grid.minor.x = element_blank(),
        panel.grid.major.x = element_line(color = "#626262", linetype = "1F", size = 0.1),
